    ...
    return(
        <div className='NewsItemBlock'>
            ...
            <Modal
                ...
                style={{
                    content: {
                        width: '1200px', // 모달창의 너비를 600px로 설정
                        height: '600px', // 모달창의 높이를 400px로 설정
                        margin: 'auto', // 모달창을 화면 가운데에 위치시킴
                    },
                }}
            >
                <h2><a href={originallink} target="_blank" rel='noopener noreferrer'>{title}</a></h2>
                <button onClick={() => setModalIsOpen(false)}>Close</button> {/* Close 버튼을 클릭하면 모달창이 닫힙니다 */}
                <iframe src={originallink} width="100%" height="80%"  title="News Article" onError={(e) => {
